...
int sz = INT_MIN;
std::atomic<double> bestVal = INT_MAX;
...
void print_distances(vector<double>& dist, unsigned int n)
{
	...
	{
		std::cout << round(dist[i] * 1000) / 1000 << "\t";
		if ((i + 1) % n == 0)
			std::cout << endl;
	}
}
...
bool read_tsp_file(char* fname)
{
	...
	std::cout << fname << " file not open" << endl;

	return false;
}

vector<double> qap_facility;
vector<double> qap_location;

bool read_qap_file(char* fname)
{
	std::ifstream file(fname);
	...
	if (file.is_open())
	{
		...
		sin >> sz;
		qap_facility.resize(sz * sz);
		qap_location.resize(sz * sz);
		int line_index = 0;
		...
		for(int i = 0; i < sz; i++)
		{
			...
			{
				sin >> qap_facility[line_index * sz + i];
			}
		...
		for (int i = 0; i < sz; i++)
		{
			...
			for (size_t i = 0; i < sz; i++)
			{
				sin >> qap_location[line_index * sz + i];
			}

			line_index++;
		}

		print_distances(qap_facility, sz);

		cout << endl;

		print_distances(qap_location, sz);
		file.close();
...
void printPermWithVal(vector<int>& data)
{
	...
	std::cout << " = " << calculateDistance(data).value << endl;
}
...
void printPerm(vector<int>& data)
{
	...
	std::cout << justDistance(data) << endl;
}
...
void work(int id, int lvl, int source, int target, vector<int> perm)
{
	...
	for (size_t i = 0; i < factorial(sz - lvl); i++)
	{
		...
		if (data.skip)
		{
			// Bez while funguje jak ma pro 12! za 321s
			while (true)
			{
...
int main()
{
	char tsp[] = "TSP_smallInput.txt";
	char qap[] = "QAP_input.txt";
